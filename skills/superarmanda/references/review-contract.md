...
Under the Claude Code sandbox (marker `SANDBOX_RUNTIME` or `CLAUDE_CODE_HOST_HTTP_PROXY_PORT`) the scrub keeps exactly the sandbox proxy URLs (`HTTP(S)_PROXY`, `ALL_PROXY` and lowercase twins; `NO_PROXY` is a bypass list, not a route, and is never kept), and only when every proxy URL points at loopback (`localhost`, `127.0.0.1`, `::1`): that proxy is the trusted egress route, not an alternate routing knob — without it the child has no network at all. Every other `*PROXY*` variable is still removed.
